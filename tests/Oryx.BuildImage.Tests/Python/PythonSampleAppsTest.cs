...
namespace Microsoft.Oryx.BuildImage.Tests
{
    ...
    public class PythonSampleAppsOtherTests : PythonSampleAppsTestBase
    {
        ...
        public void GeneratesScript_AndBuilds_WhenExplicitLanguageAndVersion_AreProvided()
        {
            ...
            var appOutputDir = $"{appDir}/output";
            var script = new ShellScriptBuilder()
                .AddBuildCommand($"{appDir} -i /tmp/int -o {appOutputDir} --platform python --platform-version {PythonVersions.Python36Version}")
                .ToString();
        ...
        public void Build_InstallsVirtualEnvironment_AndPackagesInIt()
        {
            ...
            var script = new ShellScriptBuilder()
                .AddBuildCommand(
                $"{appDir} -i /tmp/int -o {appOutputDir} --platform python --platform-version {PythonVersions.Python37Version}")
        ...
        public void Build_ExecutesPreAndPostBuildScripts_WithinBenvContext(string version)
        {
            ...
            var appOutputDir = "/tmp/app-output";
            var script = new ShellScriptBuilder()
                .AddBuildCommand($"{appDir} -i /tmp/int -o {appOutputDir} --platform python --platform-version {version}")
                .ToString();
